...
public class StrokedTextView extends TextView {
    ...
    private byte[] mStrokeColor; //ARGB
    private float mStrokeWidth;
    private byte[] mTextColor; //ARGB
    ...
    public void setProperties (Typeface typeface, byte[] strokeColorARGB, float strokeWidth, byte[] textColorARGB) {
		mTypeface = typeface;
		mStrokeColor = strokeColorARGB;
		mStrokeWidth = strokeWidth;
		mTextColor = textColorARGB;
    }

    private void init(Context context, AttributeSet attrs, int defStyle) {
    	mStrokeColor = new byte[] { (byte)255, 0, 0, 0 };
		mStrokeWidth = 1;
		mTextColor = new byte[] { (byte)255, (byte)255, (byte)255, (byte)255 };
	...
	protected void onMeasure (int widthMeasureSpec, int heightMeasureSpec) {
		...
		width += (int)mStrokeWidth * ((text.length()+1) * 0.35f);
    ...
    protected void onDraw(Canvas canvas) {
        if (mCache != null) {
            if (mUpdateCachedBitmap) {
				...
				textWidth += (int)mStrokeWidth * ((text.length()+1) * 0.35f);
                ...
                final int left = (int) (w - getPaddingRight() - textWidth + mStrokeWidth/2.0f);
                final int bottom = (h + textBounds.height()) / 2;

                // Draw the outline of the text
				mPaint.setTypeface(mTypeface);
                mPaint.setStrokeWidth(mStrokeWidth);
                mPaint.setARGB(mStrokeColor[0], mStrokeColor[1], mStrokeColor[2], mStrokeColor[3]);
                mPaint.setTextSize(getTextSize());
                mCanvas.drawText(text, left, bottom, mPaint);

                // Draw the text itself
                mPaint.setStrokeWidth(0);
                mPaint.setARGB(mTextColor[0], mTextColor[1], mTextColor[2], mTextColor[3]);
                mCanvas.drawText(text, left, bottom, mPaint);
